...
export class SmartTablePage extends BasePage {
	...
	readonly tablePageNavigationBtn: Locator;
	...
	constructor(page: Page) {
		...
		this.emailInput = this.page.locator('input-editor').getByPlaceholder('E-mail');
		this.confirmCheckmark = this.page.locator('.nb-checkmark');
		this.tableRows = this.page.locator('tbody tr');
		this.headerColumns = this.page.locator('thead').locator('tr').nth(0).locator('th');
		this.tablePageNavigationBtn = this.page.locator('.ng2-smart-pagination-nav');
	}
	...
	async findIdColumnIndex() {
		const headerColumns = await this.headerColumns.all();
		let columnIndex = -1;

		for (let i = 0; i < headerColumns.length; i++) {
			const hasClass = await headerColumns[i].evaluate((th) => th.classList.contains('id'));
			if (hasClass) {
				columnIndex = i;
				break;
			}
	...
	async openRowInEditMode(targetRow: Locator) {
		try {
			...
			throw new Error('The row to click on is not found. Please try again with another row.');
		}
	}
	...
	async navigateToTablePage(page: string) {
		await this.tablePageNavigationBtn.getByText(page).click();
	}
}
